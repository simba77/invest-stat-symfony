...
const {loading, run: confirmDelete} = useAsync((id: number) => deleteCoupon(id).then(() => {
  modal.close()
        ...
        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
          ...
          <div class="mt-2">
            <p class="text-sm text-gray-500">
              Are you sure you want to delete coupon &quot;<b>{{ modelValue.item.ticker }} - {{ formatPrice(modelValue.item.amount) }}</b>&quot;?
            </p>
          </div>
        </div>
      </div>
